...
class Nyxus:
    ...
    def featurize_dask(
        ...
        """
        print("function")
        print(type(intensity_images))
        print(type(label_images))
        intensity_images = np.squeeze(intensity_images)
        label_images = np.squeeze(label_images)
        print("shape")
        print(intensity_images.shape)
        ...
        result = da.map_blocks(self._featurize_dask, intensity_images, label_images, meta=np.array((), dtype=np.uint8)).compute()
        #result = da.map_blocks(self._featurize_dask, intensity_images, label_images, dtype='float32', chunks=(1,1,1)).compute()

        #print("Compute chunk size")
        ...
        #result = pd.concat(self.dask_results)
        
        #print(self.dask_results)
        
        dfs = []
        for block in self.dask_results:
            ...
            df = pd.concat(
                [
                    pd.DataFrame(block[0], columns=self.header[: block[0].shape[1]]),
                    pd.DataFrame(block[1], columns=self.header[block[0].shape[1] :]),
                ],
                axis=1,
            )
        ...
        df = pd.concat(
            [
                pd.DataFrame(result, columns=self._header[: result.shape[1]]),
            ],
            axis=1,
        )
    ...
    def _featurize_dask(
        ...
        #print(intensity_images.ndim)
        if(intensity_images.ndim == 0 or intensity_images.ndim == 4):
            array1 = np.array([])
        ...
        print("before featurize")
        
        header, string_data, numeric_data, error_message = featurize_montage_imp (intensity_images, label_images, intensity_names, label_names)
        
        #df1 = pd.DataFrame(string_data, columns=header[: string_data.shape[1]])
        #df2 = pd.DataFrame(numeric_data, columns=header[string_data.shape[1] :])
        ...
        self.dask_results.append([string_data, numeric_data])
        #self.dask_result.concat(df)
        #self.dask_result = pd.concat([self.dask_result, df])
        ...
        if (string_data.ndim == 1):
            result = np.append(string_data, numeric_data)
            print(result)
            return np.append(string_data, numeric_data)
        
        size = len(string_data[0]) + len(numeric_data[0])
        result = np.zeros((len(string_data), size), dtype=object)
        for i in range(0,len(string_data)):
            result[i] = np.append(string_data[i], numeric_data[i])
