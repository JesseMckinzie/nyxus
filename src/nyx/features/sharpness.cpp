...
std::vector<double> SharpnessFeature::smooth_image(const std::vector<unsigned int>& image, int rows, int cols, bool transpose, double epsilon) {
    ...
    auto apply_convolution = [&](const std::vector<double>& vec) {
        for (const auto& pix : convolve_1d(vec, kernel)) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (auto& pix: convolve_1d(std::vector<double>(in.begin() + i*cols, in.begin() + (i+1)*cols), kernel)) {
            result.push_back(pix);
        }
    }
